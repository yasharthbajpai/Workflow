...
- **AI extraction**: Gemini (`gemini-3.6-flash` by default, via `google-genai`), with a
...
cp .env.example .env
# edit .env: set DATABASE_URL to your Postgres instance, and (optionally) GEMINI_API_KEY
...
Without `GEMINI_API_KEY` set, "Scan my inbox" falls back to a small regex parser
for Uber/MakeMyTrip senders — enough to demo the duplicate-cab and flight-memo
traps, but not the itemised hotel folio or the dinner bill (those need Gemini to
read the image / itemise the invoice). Set the key to see the full pipeline.
...
- **Backend → Render**: `render.yaml` at the repo root defines the service
  ...
  Set `DATABASE_URL` (from a Render Postgres instance), `GEMINI_API_KEY`, and
  `CORS_ORIGINS` (your Vercel URL) in the Render dashboard.
...
backend/
  app/
    ...
    schemas/        Pydantic request/response + the Gemini extraction schema
    services/       policy_engine, routing, workflow, gemini_extraction, claim_builder
